...
import { leavesApi, Leave, LeaveType, BulkCreateLeaveDto } from "@/lib/api/leaves";
...
export default function LeaveEditPanel({
      ...
      if (selectedDays.length > 0 && selectedLeaveType) {
        ...
        for (const day of selectedDays) {
          ...
          try {
            ...
            if (targets.length === 0 && day.getDay() !== 0 && day.getDay() !== 6) {
  ...
  const canSave = hasChanges && (selectedLeaveType !== null || pendingRemovals.size > 0);

  // Mevcut izinleri filtrele (hedef girilmiş günler hariç)
  const filteredLeaves = existingLeaves.filter((leave) => !pendingRemovals.has(leave.date));

  return (
    <div className="bg-surface-container p-6 rounded-xl border border-outline-variant shadow-sm h-full flex flex-col">
      {/* Header */}
      ...
      <div className="mb-4">
        ...
        {isLoading ? (
          ...
          <div className="bg-surface-container-high p-3 rounded-lg">
            <p className="text-sm text-on-surface-variant italic">Henüz izin kaydı yok</p>
          </div>
        ) : (
            ...
            {filteredLeaves.map((leave) => {
              ...
              return (
                <button
                  ...
                  style={{
                    backgroundColor: isPendingRemoval ? "#9E9E9E" : config.color,
                  }}
      ...
      {/* Yeni İzin Ekle */}
      <div className="mb-4">
        <h4 className="text-sm font-semibold text-primary mb-3">Yeni İzin Ekle</h4>
        ...
        <div className="grid grid-cols-1 gap-2 mb-3">
          <button
            onClick={() => {
              const newType = selectedLeaveType === LeaveType.ANNUAL_LEAVE ? null : LeaveType.ANNUAL_LEAVE;
            ...
            className={`px-4 py-2.5 rounded-lg border-2 transition-all text-left ${
              selectedLeaveType === LeaveType.ANNUAL_LEAVE
                ...
                : "border-outline-variant bg-surface-container-highest text-on-surface-variant hover:border-blue-300"
            }`}
          >
          ...
          <button
            onClick={() => {
              const newType = selectedLeaveType === LeaveType.SICK_LEAVE ? null : LeaveType.SICK_LEAVE;
              onLeaveTypeChange(newType);
            }}
            className={`px-4 py-2.5 rounded-lg border-2 transition-all text-left ${
              selectedLeaveType === LeaveType.SICK_LEAVE
                ? "border-red-500 bg-red-50 text-red-700"
                : "border-outline-variant bg-surface-container-highest text-on-surface-variant hover:border-red-300"
            }`}
          >
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          ...
          <button
            onClick={() => {
              const newType = selectedLeaveType === LeaveType.ASSIGNMENT_LEAVE ? null : LeaveType.ASSIGNMENT_LEAVE;
              onLeaveTypeChange(newType);
            }}
            className={`px-4 py-2.5 rounded-lg border-2 transition-all text-left ${
              selectedLeaveType === LeaveType.ASSIGNMENT_LEAVE
                ? "border-purple-500 bg-purple-50 text-purple-700"
                : "border-outline-variant bg-surface-container-highest text-on-surface-variant hover:border-purple-300"
            }`}
          >
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        {/* Açıklama */}
        <p className="text-sm font-medium text-on-surface mb-2">Açıklama (İsteğe Bağlı)</p>
        <textarea
          value={note}
      ...
      >
        {hasChanges ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-on-surface-variant">Henüz değişiklik yok</span>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2 mt-auto">
        <button
          onClick={onCancel}
          className="flex-1 px-4 py-2 border border-outline-variant text-on-surface-variant rounded-lg text-sm font-medium hover:bg-surface-container-high transition-colors"
        >
          Vazgeç
        </button>
        <button
