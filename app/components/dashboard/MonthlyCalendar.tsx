    ...
    <div className="bg-surface-container p-6 rounded-xl border border-outline-variant shadow-sm">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold text-on-surface">Hedef Takvimi</h3>
        </div>
        <div className="flex items-center gap-2">
          {onEditLeaves && (
            <button
            ...
            >
              {editMode ? "Düzenlemeyi Bitir" : "İzinleri Düzenle"}
            </button>
          )}
          <button
            onClick={prevMonth}
      ...
      const isWeekend = day.date.getDay() === 0 || day.date.getDay() === 6;
            ...
            return (
              <button
                ...
                `}
                disabled={!day.isCurrentMonth || (editMode && (hasTarget || isWeekend))}
              >
                <div className="flex flex-col h-full">
                  {/* Day Number */}
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      {editMode && selectedLeaveType && !hasTarget && !hasLeave && !isWeekend && (
                        <input
                          type="checkbox"
                  ...
                  {day.targets && day.targets.length > 0 && (
                    <div className="flex-1 space-y-1 overflow-hidden mt-1">
                      {day.targets.slice(0, 3).map((target, idx) => {
                        ...
                        const projectName = target.selectedProjects?.[0] || target.customProject || "Proje";
                        ...
                        return (
                          ...
                          >
                            <div className={`w-1 h-1 rounded-full ${statusColor} flex-shrink-0`} />
                  ...
                  {day.target && day.target.note && (
                    <div className="mt-auto pt-1 text-xs text-on-surface-variant truncate" title={day.target.note}>
                      {day.target.note}
                    </div>
                  )}
